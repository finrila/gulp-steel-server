...
module.exports = function proxyMiddleware(options) {
	...
	for (var key in options) {
		if (key.indexOf('.') !== -1) {
			...
			if (typeof proxyHost === 'string') {
				var proxyHost_split = proxyHost.split(':');
				var proxyHost_port = 80;
				if (proxyHost_split.length === 2) {
					proxyHost = proxyHost_split[0];
					proxyHost_port = parseInt(proxyHost_split[1]);
				}
				proxyHost = {
					hostname: proxyHost,
					port: proxyHost_port
	...
	return function(req, resp, next) {
		...
		opts.headers.host = proxyHost.hostname;
